...
| 6 | 🟡 Warning | Hero | Both | Visual Completeness | Three.js globe is not rendering. The hero displays a particle/star-field background only, with no interactive 3D globe. This is the primary visual centrepiece described in the architecture. | hero-desktop.png | Debug the Three.js `WebGLRenderer` initialisation in the hero animation; check for console errors (`WebGL context lost`, canvas not appended, missing geometry). Verify `Hero.jsx` mounts the canvas correctly. |
